      ...
      <div className="bg-[#c5e0db] rounded-xl p-2 shadow-lg flex flex-col items-center">
        <div className='container flex gap-6'>
          <div className="w-full mt-1 p-4 bg-gradient-to-br from-[#61BA84] to-[#3A8D5C] rounded-xl shadow-[0_10px_20px_rgba(0,0,0,0.3)] transition-all duration-400 hover:-translate-y-2 hover:scale-[1.03] hover:shadow-[0_20px_40px_rgba(0,0,0,0.35)] relative overflow-hidden group">
            ...
            {/* Stats Container */}
            <div className="flex justify-around items-end mb-2.5 gap-6">
              {/* Today */}
          ...
          <div className="w-full mt-1 p-2 bg-gradient-to-br from-[#61BA84] to-[#3A8D5C] rounded-xl shadow-[0_10px_20px_rgba(0,0,0,0.3)] transition-all duration-400 hover:-translate-y-2 hover:scale-[1.03] hover:shadow-[0_20px_40px_rgba(0,0,0,0.35)] relative overflow-hidden group">
            {/* Title */}
              ...
              <div className="flex flex-col items-start">
                {most5LaunchesLastWeek.map((item, idx) => (
                  <div
                    ...
                    style={{ minWidth: 120 }}
                  >
                    <span className="text-white font-bold text-base uppercase mr-1">• {item.buyer_name}</span>
                    <span className="text-white font-semibold text-base">➜ {item.total_complaints}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            {/* Animated moving bars in a box */}
            <div className="absolute right-2 bottom-13 w-17 h-14 flex items-center justify-center">
              <div className="relative w-17 h-14 bg-white/10 rounded-md flex items-end justify-between px-1 overflow-hidden">
                {[0, 1, 2, 3, 4].map((i) => (
                  <div
                    key={i}
                    className="w-1.5 rounded bg-[#ff9232] animate-bar-bounce"
                    style={{
                      height: '90%',
